...
# with open('fileList.txt', 'wb') as file:
#     for f in fileList:
...
for fileName in newFileList:
    ...
    for tree in d.nodeList:
        ...
        with open("preprocessed_dataset.txt", "w", encoding="utf-8") as output_file:
            for fileName in newFileList:
                d = Document(fileName)
                for tree in d.nodeList:
                    for chunkNode in tree.nodeList:
                        ...
                        if head != 'NULL':
                            if key[0] == "af":
                                output_file.write(f"{head} {chunkNode.name} {post} {chunkNode.parent} {chunkNode.parentRelation} {ai} {srl} {pred}\n")
                            else:
                                if head != '':
                                    output_file.write(f"{head} NULL\n")
